...
**Input tokens** — everything Claude reads: your prompt, history, instructions, files, tool schemas, logs, test output. This is why a short prompt late in a long session can still be expensive — Claude is processing everything that came before it too.
...
3. **Keep CLAUDE.md lean.** Only include instructions that apply broadly — aim for under roughly 200 lines. Move specialized, occasional workflows into on-demand skills instead.
