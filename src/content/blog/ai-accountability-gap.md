...
None of these are exotic. They're the same disciplines good engineering teams have always needed. The difference is that AI-assisted development makes it *possible* to skip them entirely and still ship. That means, for the first time, skipping them is no longer self-correcting. The code still works. The bugs still get "fixed." And the understanding just isn't there when you need it.
